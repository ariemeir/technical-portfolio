...
public class ServiceTerminal extends Activity implements View.OnClickListener{
	
	private static ServiceTerminal _instance=null;
	...
	static float voltageAmplitude=0.0f;
	...
	static int rotationDirection = ConfigDB.ROTATION_CW;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final Handler mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                //units here are mA so we divide by 1000
                float currentmA = currentAmplitude*1000.0f;
                tv.setText(new DecimalFormat("#.##").format(currentmA));
                ...
                tv.setText(Float.toString(voltageAmplitude));
    ...
    private void resetCounters() {
    	...
    	voltageBufferWraparound=false;
    	currentBufferWraparound=false;
    	currentAmplitude=0.0f;
    	voltageAmplitude=0.0f;
	...
	public void drawChart()
    {
     ...
     public static void updateVoltageData(float value)
     {
    	...
    	synchronized(voltageTimes)
    	{
			...
			voltageBuffer[voltageBufferPosition]= value + (float)rnd.nextGaussian()/100000;
     ...
     public void setButtonHandlers()
     {
         ...
         Switch poweron = (Switch)this.findViewById(R.id.switch_poweron);
         poweron.setOnCheckedChangeListener( new CompoundButton.OnCheckedChangeListener() 
         {
             public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) 
             {
            	 	Toast.makeText(ServiceTerminal.this, "Monitored switch is " + (isChecked ? "on" : "off"),Toast.LENGTH_SHORT).show();
            	 	if (isChecked==true)
            	 	{
            	 		BlueTerm.getInstance().send("atmotoron\n\r".getBytes());
            	 		//the HW has a 256 byte uart buffer which can overrun if too much is being sent at once
            	 		Utility.delay(100);
            	 		BlueTerm.getInstance().send("atreset\n\r".getBytes());
            	 		Utility.delay(100);
            	 		BlueTerm.getInstance().send("atupdateinterval 500\n\r".getBytes());
            	 		Utility.delay(100);
            	 		//enable the updates
            	 		BlueTerm.getInstance().send("atupdate 1\n\r".getBytes());
            	 		
            	 		
            	 	}else
            	 		BlueTerm.getInstance().send("atmotoroff\n\r".getBytes());
            	 	//BlueTerm.getInstance().send("atupdateinterval 5000\n\r".getBytes());
         ...
         ToggleButton tb = (ToggleButton) this.findViewById(R.id.directionToggle);
    ...
    public void onClick(View v)
    {
    	...
    	switch(clickOriginID) 
    	{
    		...
    		case R.id.plus_button:
    			System.out.println("+++++++++++++++++++++++++++++");
    			//plus button is elongation of the screw
    			BlueTerm.getInstance().send("atinc\n\r".getBytes());
    			BlueTerm.getInstance().send("atblink\n\r".getBytes());
    			break;
    		case R.id.minus_button:
    			System.out.println("-----------------------------");
    			//combine two commands: atdec and atblinkmeasureVoltage(&phyParams.voltage);
    			//minus button is shortening 
    			BlueTerm.getInstance().send("atdec\n\r".getBytes());
    			BlueTerm.getInstance().send("atblink\n\r".getBytes());
    			break;
    		case R.id.directionToggle:
    	         rotationDirection = 1-rotationDirection;
    	    	 //stop the motor and then switch the direction
    	         BlueTerm.getInstance().send("atspeed 0 \n\r".getBytes());
    	         Utility.delay(100);
    	         if (rotationDirection==ConfigDB.ROTATION_CW)
    	        	 BlueTerm.getInstance().send("atcw\n\r".getBytes());
    	         else
    	        	 BlueTerm.getInstance().send("atccw\n\r".getBytes());
    	        	 
    	         BlueTerm.getInstance().send("atspeed 40 \n\r".getBytes());
	...
	public static void updateVoltageAmplitude(float vamp) {
		voltageAmplitude = vamp;
		System.out.println("Updating voltage amplitude to "+vamp);
	}




	public static void updateCurrentAmplitude(float camp) {
		currentAmplitude = camp;
		System.out.println("Updating current amplitude to "+camp);
	}
