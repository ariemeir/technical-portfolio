...
void testUart1InLoopback(void)
{
    unsigned int cc = uart1_getc();  
    if ( cc & UART_NO_DATA || cc&0xFF00 )
    {   
    }   
    else
    {   
      uart1_putc((unsigned char)cc);
      //blink(1);
    }   
}
...
Purpose:  waits till there is a valid input on the uart
Returns:  the character read from the uart
**************************************************************************/

unsigned char uart_getc_blocking(void)
{
  int x=0;
  int retries=0;
  ...
  do 
  {
    ...
    retries++;
    //if (x!=UART_NO_DATA)
    //  printf("x=0x%x, mask : 0x%x \n",x,(x&0xFF00));
  }
  while (!IS_VALID_UART_INPUT(x));
