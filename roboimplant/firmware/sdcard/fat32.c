...
//transmitHex(INT, bytesPerSector); uart_putc(' ');
sectorPerCluster = bpb->sectorPerCluster;
//transmitHex(INT, sectorPerCluster); uart_putc(' ');
...
//transmitHex(LONG, totalClusters); uart_putc(' ');
...
while(1)
{
  ...
  cluster = getSetNextCluster (cluster, GET, 0);
  if(cluster == 0) {printf("Error in getting cluster\n"); return 0;}
}
return 0;
}
...
if(j>8) {printf("Invalid fileName..\n"); return 1;}
...
if(j == 1) 
{
  printf(" File already exists, appending data..\n"); 
  appendFile = 1;
...
printf("Since we don't have RTC, date/time is 0.\n");
error = 0; 
dateFAT=0;
timeFAT=0;
...
//Function: to create a file in FAT32 format in the root directory if given 
//			file name does not exist; if the file already exists then append the data
//Arguments: pointer to the file name, buffer and length of the buffer
//return: none
//************************************************************************************
void writeFileFromBuffer (unsigned char *fileName,unsigned char* buffer,unsigned int length)
{
unsigned char j, data, error, fileCreatedFlag = 0, start = 0, appendFile = 0, sectorEndFlag = 0, sector=0;
unsigned int i, firstClusterHigh=0, firstClusterLow=0;  //value 0 is assigned just to avoid warning in compilation
struct dir_Structure *dir;
...
while(1)
{
   ...
   //Arie : this is a binary stream, we just dump everything in.
	 /*if(data == 0x08)	//'Back Space' key pressed
	 { 
	   if(i != 0)
	   { 
