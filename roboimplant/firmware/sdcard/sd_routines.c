...
unsigned int retry=0 ;
 ...
 // mark the /CS signals for both the micro-sd and the sd card as outputs
 sbi(DDRB,DDB3);
 sbi(DDRA,DDA3);
 ...
 for(i=0;i<10;i++)
  ...
  do
  {
     ...
     if(retry>0x20) 
     {
       //printf("Bailing out, no card here\n");
       return 1;   //time out, card not detected
     }
   
     //printf("Getting 0x%x\ as response\n",response);  
  } while(response != 0x01);
...
if(SDHC_flag == 0)		
{
  ...
  SD_CS_DEASSERT;
  //printf("Returning response : 0x%x\n",response);
...
unsigned char SD_writeSingleBlock(unsigned long startBlock)
{
  ...
  if(response != 0x00) 
  {
     printf("Returning from %d with response : 0x%x (should be zero here)\n",__LINE__,response);
  ...
  {                              //AAA='101'-data rejected due to CRC error
    ...
    printf("Returning from %d, response : 0x%x, masked : 0x%x \n",__LINE__,response, (response&0x1f));
    return response;
  }
...
void SD_displayCardInfo(FILE* stream)
{
  switch (cardType)
  {
    case 1:fprintf(stream,"Standard Capacity Card (Ver 1.x) Detected!\n");
         break;
    case 2:fprintf(stream,"High Capacity Card Detected!\n");
         break;
    case 3:fprintf(stream,"Standard Capacity Card (Ver 2.x) Detected!\n");
         break;
    default:fprintf(stream,"Unknown SD Card Detected!\n");
         break; 
  }
}
